...
export function handleCCSmartWalletAdminUpdated(event: CCSmartWalletAdminUpdatedEvent): void {
  let entity = new CCSmartWalletAdminUpdated(event.transaction.hash.toHex() + "-" + event.logIndex.toString())
...
export function handleResponseTxWasSent(event: ResponseTxWasSentEvent): void {
  let entity = new ResponseTxWasSent(event.transaction.hash.toHex() + "-" + event.logIndex.toString())
...
export function handleSignerUpdated(event: SignerUpdatedEvent): void {
  let entity = new SignerUpdated(event.transaction.hash.toHex() + "-" + event.logIndex.toString())
